...
:class:`MUSEreduce.musereduce` is an easy-to-use python Class used as wrapper for the VLT/`MUSE data reduction pipeline`_ and does not replace the core functionalities of the pipeline provided by ESO. In order to function properly with this version of :class:`MUSEreduce.musereduce` we recommend to install the pipeline version v2.8.9 found under: https://www.eso.org/sci/software/pipelines/muse/. This version of :class:`MUSEreduce.musereduce` has been tested with version v2.8.9 of the data reduction pipeline.

.. _MUSE data reduction pipeline: https://www.eso.org/sci/software/pipelines/muse/

To run :class:`MUSEreduce.musereduce` all of the raw data must be stored in a folder named `user_path/raw/`. The script has to point to `user_path` by setting the keyword ``rootpath`` of the :obj:`json` config file to the parent directory.
...
.. json:object:: config
   ...
   :property cleanup: paraneters set to clean up the folders
   :proptype cleanup: :json:object:`cleanup`
...
.. json:object:: global
   ...
   :property auto_create_OB_folders: If ``auto_create_OB_folders`` = :obj:`True` OB folders are atomatically created. This is particualrly needed when a clean run wants to be accomplished.
   :proptype auto_create_OB_folders: bool
   :options auto_create_OB_folders: false, true, default=true
   ...
   :property exp_list: A user list with the identifiers of the dither positions, in case only a subset shall be collected and copied to the final folder
                        to be used for the combination. If left empty all dither positions are used to create the final data cube.
   :proptype exp_list: list
   :options exp_list: default=[]

.. json:object:: cleanup

   :property execute: Must be set :obj:`True` if this step should be execute
   :proptype execute: bool
   :options execute: false, true, default=false

   :property include_std: Must be set :obj:`True` if the standard star folder should be removed
   :proptype include_std: bool
   :options include_std: false, true, default=false

   :property include_combined: Must be set :obj:`True` if the combined exposure folder should be removed
   :proptype include_combined: bool
   :options include_combined: false, true, default=false

   :property include_calibrations: Must be set :obj:`True` if all calibration and intermediate products should be removed. This can be executed to save harddrive space after a run has finished.
   :proptype include_calibrations: bool
   :options include_calibrations: false, true, default=false
...
.. json:object:: sci_basic
   ...
   :property execute_sci: Must be set :obj:`True` if the SCIENCE object should be reduced
   :proptype execute_sci: bool
   :options execute_sci: false, true, default=true
...
.. json:object:: exp_align
   ...
   :property scrmin: the minimum number of sources that has to be used to align cubes
   :proptype scrmin: int
   :options scrmin: default=5

   :property scrmax: the maximum number of sources that can be used to align cubes
   :proptype scrmax: int
   :options scrmax: default=80
