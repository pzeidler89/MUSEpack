...
from MUSEpack.rv_utils import *


__author__ = "Peter Zeidler (zeidler@stsci.edu)"
__revision__ = 20191129
...
class RV_spectrum:
    ...
    def init_cat(self,line_file):
        ...
        temp = ascii.read(line_file)
        
        self.logger.info('Adding lines: '+str(temp['name'].data))
        
        
        d = {'l_lab': temp['lambda'],'l_start': temp['start'],'l_end': temp['end'],\
        'l_fit':np.empty_like(temp['name']),'a_fit':np.empty_like(temp['name']),\
        'sg_fit':np.empty_like(temp['name']),'sl_fit':np.empty_like(temp['name']),\
        'cont_order':np.empty_like(temp['name']),'RV':np.empty_like(temp['name']),\
        'eRV':np.empty_like(temp['name']),'used':np.empty_like(temp['name'])}
        
        self.cat = pd.DataFrame(d,index = temp['name'].data)
        
        
    def line_fitting(self,input_cat,line_idxs,niter = 5, contorder = [1], n_CPU = -1, resid_level = 0.03, max_contorder = 2, adjust_preference = 'contorder'):
        ...
        # fig=plt.figure('pyspeckit_plot_')

        
        results = [dask.delayed(line_fitting_dask)(self,input_cat,ldx,niter,resid_level,max_contorder,adjust_preference) for ldx in np.array(line_idxs)]
        results = dask.compute(*results,num_worker=n_CPU,scheduler='processes')
        
        # results = line_fitting_dask(self,input_cat,'HeI4922',niter,resid_level,max_contorder,adjust_preference)
        # results = [results]
    ...
    def rv_fit(self, guesses, niter = 10000, line_exclude = False, line_sigma = 3, n_CPU = -1):
        ...
        self.logger.info('Settings: niter='+str(niter)+'; excluded lines: '+str(line_exclude)+'; sigma RV for excluding lines: '+str(line_sigma))
        ...
        if line_exclude:
            for line in line_exclude:
                ind = np.where(line != line_name)[0]
                line_name = line_name[ind]
                l_lab = l_lab[ind]
                l_fit = l_fit[ind]
                fwhm_v = fwhm_v[ind]
        ...
        remaining_lines=sigma_clip(v, sigma = line_sigma, stdfunc=MAD)
        ...
        pp_final_init = ppxf.ppxf(log_template_f,log_spec_f,log_spec_err,velscale_spec,guesses,degree=-1,clean=False,mask=mask,quiet=True)
        pp_final_init.plot()
        plt.tight_layout()
